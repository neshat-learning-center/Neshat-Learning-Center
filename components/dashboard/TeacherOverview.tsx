...
import { Panel, Card, Empty } from "./primitives";
...
export function TeacherOverview({
  ...
  return (
    <div className="flex flex-col gap-12">
      <Panel id="classes" title={d.classes}>
            ...
            <table className="w-full text-start text-sm">
              <thead>
                <tr className="border-b border-line text-muted">
                  ...
                  <th className="px-5 py-3 text-start font-medium">{d.classes}</th>
                  <th className="px-5 py-3 text-start font-medium">{d.level}</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-line">
                {data.students.map((s, i) => (
                  <tr key={i}>
                    ...
                    <td className="px-5 py-3 text-ink-soft">{s.klass}</td>
                    <td className="px-5 py-3 text-ink-soft">{s.level}</td>
                  </tr>
                ))}
              </tbody>
            </table>
